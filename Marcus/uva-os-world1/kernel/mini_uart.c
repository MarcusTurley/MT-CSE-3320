...
void uart_send (char c) {
    uint32_t _status;
	while(1) {
            _status = get32(AUX_MU_LSR_REG);
            if(IS_TRANSMITTER_IDLE(_status))
                break;
	}
...
void uart_irq(void) {
    ...
    if (IS_RECEIVE_INTERRUPT(iir)) {
        while (1) {
            ...
            if (get32(AUX_MU_LSR_REG) & (1 << 0)) // LSR bit0 = FIFO empty
                break;

            c = get32(AUX_MU_IO_REG) & 0xFF; // read one char
            V("char %d", c); 

            // Q12: handle key press
            test_ktimer2(c);
        }
    }
}
