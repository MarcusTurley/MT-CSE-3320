...
unsigned long current_counter() {
	return (unsigned long)get32(TIMER_CLO) 
		| ((unsigned long)get32(TIMER_CHI) << 32);
}
...
static int adjust_sys_timer(void)
{
	...
	// write low 32 bits of next expiration to compare register
    put32(TIMER_C1, next & 0xFFFFFFFF); 
	return 0; 
}
	...
	for (int t = 0; t < N_TIMERS; t++) {
		...
		if (timers[t].elapseat <= cur) { // should fire  
			...
			if (ret==1) { // restart the ktimer in place
				timers[t].elapseat = cur + timers[t].delayms * 1000;
				adjust_sys_timer(); 
			} else 
				timers[t].handler = 0; 
		}		
