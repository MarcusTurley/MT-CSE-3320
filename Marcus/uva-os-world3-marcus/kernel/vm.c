...
static void _create_block_map(unsigned long *tbl, 
  ...
  unsigned long flags, int shift) {
	...
	unsigned long idx_start = (start >> shift) & (PTRS_PER_TABLE - 1);
  unsigned long idx_end = (end >> shift) & (PTRS_PER_TABLE - 1);

  for (unsigned long i = idx_start; i <= idx_end; i++) {
    tbl[i] = phys | flags;
    phys += (1UL << shift);
  }
}

// Map 1GB blocks (PUD level)
#define create_block_map_supersection(tbl, phys, start, end, flags) \
  _create_block_map(tbl, phys, start, end, flags, PUD_SHIFT)

// Map 2MB blocks (PMD level)
#define create_block_map_section(tbl, phys, start, end, flags) \
  _create_block_map(tbl, phys, start, end, flags, PMD_SHIFT)
...
void create_kern_pgtables(void) {
  ...
  unsigned long *pmd2 = pgd + 3*PTRS_PER_TABLE;
	...
	create_block_map_section(pmd2, DEVICE_LOW, VA_START + DEVICE_LOW, 
...
void create_kern_idmap(void) {
	...
	create_table_entry(pgd, PHYS_BASE, PGD_SHIFT, 1);

	//1. kernel mem (PUD). Phys addr range: PHYS_BASE, +PHYS_SIZE
	create_block_map_supersection(pud,
		PHYS_BASE, 
