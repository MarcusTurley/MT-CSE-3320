...
#define TIMER_TICKS_PER_MS 1000UL // addition
...
unsigned long current_counter() {
	...
	return ((unsigned long)hi << 32) | lo; /* STUDENT_TODO: replace this */
}
...
void ms_delay(unsigned ms) {
	...
	unsigned long start = current_counter();
    unsigned long wait = (unsigned long)ms * cycles_per_ms;
	while (current_counter() - start < wait)
        ;
}
...
void us_delay(unsigned us) {
	...
	unsigned long start = current_counter();
    unsigned long wait = (unsigned long)us * cycles_per_us;
...
void sys_timer_irq(void) 
{
	...
	for (int t = 0; t < N_TIMERS; t++) {
		...
		if (timers[t].elapseat <= cur) { // should fire  
			...
			ret = h(t, timers[t].param, timers[t].context); /* STUDENT_TODO: replace this */
			if (ret==1) { // restart the ktimer in place
				timers[t].elapseat = cur + timers[t].delayms * TIMER_TICKS_PER_MS; /* STUDENT_TODO: replace this */
				adjust_sys_timer(); 
			} else 
				timers[t].handler = 0; 
		}		
